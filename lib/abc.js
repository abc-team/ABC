...
function ABC(abcJSON) {
    ...
    self._configs = abcJSON.configs ? _.clone(abcJSON.configs) : {};

    self._vars = abcJSON.vars ? _.clone(abcJSON.vars) : {};

    self._compileConfigs();
...
_.extend(ABC.prototype, events.EventEmitter, {
     ...
     * @return {Object}
     */
    getConfig: function(type, name, data) {
        var self = this;
        var config = self._compiledConfigs;


        if (config[type] && config[type][name]) {

            var compiledConfig = config[type][name];

            var view = self._vars;
            if (data) {
                view = _.defaults(data, view)
            }
            return Utils.configUtil.render(compiledConfig, view);
        } else {
            return null;
        }
    },
     ...
     * @private
     */
    _compileConfigs: function (json) {
        //deliver configs
        var self = this;
        self._compiledConfigs = Utils.configUtil.compile(self._configs);
        return self;
    },
