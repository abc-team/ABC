...
function ABC(abcJSON) {
    ...
    self.json = abcJSON;
    self.configs = abcJSON.configs ? _.clone(abcJSON.configs) : {};
    self.vars = abcJSON.vars ? _.clone(abcJSON.vars) : {};
...
_.extend(ABC.prototype, events.EventEmitter, {
    ...
    getConfig: function(type, name) {
        var self = this;
        var config = self.configs;

        if (config[type] && config[type][name]) {
            var taskConfig = config[type][name];
            return utils.embedObject(taskConfig, self.vars);
        } else {
            return null;
        }
    },
    ...
    _initConfigs: function (json) {
        //deliver configs
        var config = utils.embedObject(json.configs || {}, json['var'] || {});
        var self = this;
